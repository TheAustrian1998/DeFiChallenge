...
describe("Challenge 1.2", function () {
    ...
    it("Should swap wETH for DAI successful...", async function(){
        let WETHBalanceBeforeSwap = ethers.utils.formatUnits(await this.swapperUNIV2.connect(whaleSigner).viewBalance(WETHAddress));
    ...
    it("Should withdraw DAI successful...", async function(){
        async function withdraw(whale, swapperUNIV2) {
            const amounts = ["2100","2200","2500","2600","2800"];
            const random = Math.floor(Math.random() * amounts.length);
